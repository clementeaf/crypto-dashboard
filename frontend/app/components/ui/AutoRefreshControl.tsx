...
import { AutoRefreshControlProps } from '~/types/types';
    ...
    <div className="flex items-center gap-2 p-2 bg-gray-100 dark:bg-gray-800 rounded">
      <label className="flex items-center cursor-pointer" aria-disabled={disabled}>
        <div className="relative">
          ...
          />
          <div
            className={`block w-10 h-6 rounded-full ${disabled ? 'bg-gray-300 dark:bg-gray-600' :
                autoRefresh ? 'bg-gray-600 dark:bg-gray-400' : 'bg-gray-400 dark:bg-gray-600'
              }`}
          ></div>
          <div
            className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${autoRefresh ? 'transform translate-x-4' : ''
              }`}
          ></div>
        </div>
      ...
      <select
        className={`text-sm p-1 ${disabled ? 'bg-gray-200 dark:bg-gray-600 cursor-not-allowed' :
